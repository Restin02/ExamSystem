...
const StaffDashboard = () => {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            alert("Upload failed. Please ensure Pillow is installed on the server.");
        } finally {
            setLoading(false);
        }
    };

    // Helper to calculate free periods from the timetable
    ...
    if (!data || !data.profile) {
        return (
            <div className="error-container" style={{ textAlign: 'center', marginTop: '50px' }}>
                <p>Unable to load profile data. Please login again.</p>
        ...
        <div className="staff-dashboard-root">
            ...
            <div className={`side-nav ${menuOpen ? 'open' : ''}`}>
                ...
                <div className="nav-links">
                    <button onClick={() => alert("Redirecting to Profile Settings...")}>Profile Settings</button>
                    <button onClick={() => setMenuOpen(false)}>View My Schedule</button>
                    ...
                    <div className="profile-section">
                        ...
                        <div className="profile-info">
                            ...
                            <div className="meta-grid">
                                <span><strong>Grade:</strong> {data.profile.grade}</span> 
                                <span><strong>Dept:</strong> {data.profile.department}</span>
                                <span><strong>Branch:</strong> {data.profile.branch}</span>
                    ...
                    <div className="table-responsive">
                        <table className="staff-view-table">
                            <thead>
                                <tr>
                                    <th>Day</th>
                                    {[1, 2, 3, 4, 5, 6].map(p => <th key={p}>Period {p}</th>)}
                                </tr>
                            </thead>
                            <tbody>
